"""
Bayesian Subspace Zeroth-Order (BSZO) Optimizer V4 for LLM Fine-tuning
...
V4 difference from V3: adaptive sampling uses suggest_direction() for all samples.
"""
...
class BSZOOptimizer:
    ...
    def __init__(self,
        ...
        print(f"[BSZO-V4] n={self.n:.2e}, dim={self.fixed_subspace_dim}, samples={self.num_samples}")
        print(f"[BSZO-V4] lr={self.learning_rate:.2e}, eps={self.zo_eps:.2e}, fwd/step={fwd_count}")
    ...
    def _bayesian_gradient_estimation_efficient(self, model, inputs,
        ...
        for sample_idx in range(self.num_samples):
            if self.adaptive_sampling and sample_idx >= dim:
                ...
                d_sub = self.bayes.suggest_direction()
            else:
    ...
    def optimize_on_batch(self, model, inputs, maxiter: int = 1) -> float:
        ...
        for _ in range(maxiter):
            ...
            # Gradient descent update (use posterior mean directly, no confidence weighting)
            for i, seed in enumerate(seeds):
