#!/bin/bash
# =============================================================================
# OPT-13B ZO Fine-tuning Experiments
...
# Datasets (SST2 and RTE already completed)
DATASETS=("TREC" "WIC")
...
run_zo_sgd() {
    local TASK=$1
    # TREC: 5e-6 already done
    if [ "$TASK" == "TREC" ]; then
        local LRS=("1e-6" "5e-7" "1e-7" "5e-8")
    else
        local LRS=("5e-6" "1e-6" "5e-7" "1e-7" "5e-8")
    fi
...
run_zo_adam() {
    ...
    for lr in "${LRS[@]}"; do
        ...
        export WANDB_PROJECT="zo_adam_${TASK,,}_opt13b_ft"
...
run_hizoo() {
    local TASK=$1
    # TREC: 1e-5, 5e-6 already done
    if [ "$TASK" == "TREC" ]; then
        local LRS=("1e-6" "5e-7" "1e-7")
    else
...
run_bszo_v4() {
    ...
    for lr in "${LRS[@]}"; do
        ...
        export WANDB_PROJECT="bszo_v4_${TASK,,}_opt13b_ft"
...
# =============================================================================
echo "Starting OPT-13B ZO Fine-tuning Experiments"
echo "============================================="
